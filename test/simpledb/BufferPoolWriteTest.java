	...
	class HeapFileDuplicates extends HeapFile {
		...
		public ArrayList<Page> insertTuple(TransactionId tid, Tuple t)
			...
			for (int i = 0; i < duplicates; i++) {
				...
				bw.close();
				/**
				 * FIXME: 这里原先的写法是直接new一个page，应该是有bug的？ write回磁盘后应该从bufferPool加载进内存，不然这个数据不会被存下来
				 HeapPage p = new HeapPage(new HeapPageId(super.getId(), super.numPages() - 1), HeapPage.createEmptyPageData());
				 */
				HeapPage p = (HeapPage) Database.getBufferPool()
						.getPage(tid, new HeapPageId(super.getId(), super.numPages() - 1), Permissions.READ_WRITE);
